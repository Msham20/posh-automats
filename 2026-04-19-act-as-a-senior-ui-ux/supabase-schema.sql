...
create policy "admin full testimonials"
on public.testimonials
for all
to authenticated
using (true)
...
select * from (
  values
    (
      ...
      'Automation',
      'Custom quote',
      'Operator-friendly automation controls with reliable process visibility.',
      1,
      true
    ),
    (
      ...
      'SPM',
      'Project based',
      'Custom-built machines for machining, assembly, and repeatable industrial operations.',
      2,
      true
    ),
    (
      ...
      'Inspection',
      'Custom quote',
      'Low-cost vision and positioning setups for process verification and quality control.',
      3,
      true
    )
...
select * from (
  values
    (
      'Automation Cell',
      'image',
      'assets/logo-posh-automats.jpg',
      'Starter image used until project photos are connected.',
      'Automation',
      1,
      true
    ),
    (
      'SPM Demo Reel',
      'video',
      'https://example.com/demo-machine.mp4',
      'Replace this with a real MP4 or WebM URL after setup.',
      'Media',
      2,
      true
    )
...
where not exists (
  select 1 from public.gallery_items existing
  where existing.title = seed.title
);
...
select * from (
  values
    (
      ...
      'Automotive Supplier',
      'The team approached automation practically and helped us reduce manual intervention.',
      1,
      true
    ),
    (
      'Production Manager',
      'Assembly Unit',
      'Clear engineering support, dependable delivery, and better production confidence.',
      2,
      true
    )
